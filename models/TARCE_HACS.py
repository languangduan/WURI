...
在 v1（M1~M4）基础上的追加修复：
[M5] known_bias：可学习偏置，初始化为 -1.0
     原版：known_logit = (max_prob - 1/C) * known_scale
          max_prob 在 prototype init 后对所有样本都 > 1/C
          → known_logit 永远 > 0 → risk_score 永远 > 0.5
     修复：known_logit = (max_prob - 1/C) * known_scale + known_bias
          known_bias 初始 -1.0，让大多数样本初始 risk_score < 0.5
          训练过程中 known_bias 对 harm 样本被 binary loss 推向正值
          对 benign 样本被 util loss 保持负值

[M6] score_w 中心化：减去 batch 均值
     原版：score_w = A_t @ proto_w.t()，全正，类间差异小
     修复：score_w -= score_w.mean(dim=1, keepdim=True)
          中心化后 harm 样本的匹配类得正分，非匹配类得负分
          benign 样本各类得分接近 0（均匀分布）
"""
...
# ════════════════════════════════════════════════════════════
# CoActivationComputer + CoActState
# ════════════════════════════════════════════════════════════
...
class AttackPrototypeBank(nn.Module):
    ...
    def get_proto_weights(self):
        # [M4] softplus(x) - ln2：消除基线，proto_weights=0 时激活值=0
    ...
    def match_score(self, coact_state, span_t,
        ...
        A_t     = coact_state.full_A        # [B, N]
        proto_w = self.get_proto_weights()  # [C, N]，值域 (-ln2, +∞)

        score_w = A_t @ proto_w.t()         # [B, C]

        # [M6] score_w 中心化：减去每个样本在所有类上的均值
        # 效果：harm 样本的匹配类得正分，非匹配类得负分
        #       benign 样本各类得分接近 0
        score_w = score_w - score_w.mean(dim=1, keepdim=True)

        score_coact = coact_state.frob_inner_lowrank(
            self.get_proto_coact_U())        # [B, C]
...
class TRACE(nn.Module):
    def __init__(self, model_name_or_path: str,
        ...
        # [M5] known_bias：初始化为 -1.0
        # 让大多数样本初始 risk_score < 0.5
        # binary loss 会把 harm 样本的有效路径推向正值
        # util loss 会把 benign 样本保持在负值
        self.known_bias = nn.Parameter(torch.tensor(-1.0))

        # 兼容旧代码（stage1 freeze 用到）
    ...
    def forward_trajectory(self, input_ids_seq, attention_mask_seq,
        ...
        match_scores = torch.stack(match_scores_list, dim=1)   # [B, T, C]
        zs_scores    = torch.stack(zs_scores_list,   dim=1)    # [B, T]

        # ── [M2] known_logit ──────────────────────────────────────────
        # match_scores 已经在 match_score() 里做了中心化（[M6]）
        # 所以 softmax 后 max_prob 对 harm 样本更高，对 benign 更接近 1/C
        match_probs = F.softmax(match_scores, dim=-1)           # [B, T, C]
        max_prob    = match_probs.max(dim=-1).values            # [B, T]

        # [M5] 加入 known_bias（初始 -1.0）
        # known_logit 可正可负，初始偏向负值（benign 方向）
        ...
        # ── step_scores & risk_score ──────────────────────────────────
        step_scores = torch.sigmoid(known_logit + zs_bonus)    # [B, T]
