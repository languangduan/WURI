...
def trajectory_usage_pool(step_weights, step_mask, eps=1e-12):
    """[B,T,K] + [B,T] → [B,K] 归一化 trajectory-level atom usage"""
...
class TRACEStage1TrajectoryDataset(Dataset):
    """
    v7 变更：
      1. 新增读取 meta.lang 字段（en/zh），用于跨语言分析
      2. eval_unseen_hazards_only 对 safedialbench 样本豁免过滤，
         防止 val 集中 SDB 样本被全部丢弃
      3. 统计输出新增 by_lang / by_source 分布
    """
    def __init__(
        self,
        jsonl_path,
        ...
        for ridx, record in enumerate(raw):
            ...
            # 训练集：排除指定 harmful hazard
            if split == "train" and binary_label == 1 and hazard_type in exclude_train_hazards:
                dropped += 1
                continue

            # v7 Fix: eval_unseen_hazards_only 对 SDB 样本豁免
            # 原逻辑会把所有 SDB val 样本（hazard ∈ {8,9,0}）全部丢弃
...
def stage1_collate_fn(batch):
    ...
    return {
        ...
        "langs":            [b["lang"]   for b in batch],   # v7 新增
    }
...
def loss_recon_weighted(
    ...
    """
    v7: Cosine + MSE reconstruction loss，对 benign step 上调权重。
    train 集正负比约 8:1，neg_weight=7.7 使 benign 步骤的重建误差
    贡献与 harmful 步骤持平，防止重建分布偏向 harmful。

    flat_binary: [N_valid_steps]，每步对应的 trajectory binary 标签，
                 由调用方从 batch binary 广播到 step 维度后传入。
    """
...
def _sample_triplet_pairs(y: torch.Tensor, device: torch.device):
    """
    给定标签向量 y [N]，返回 (anchor_idx, pos_idx, neg_idx)。
    每个 anchor 随机配一个同类 pos 和一个异类 neg。
    """
...
def loss_traj_triplet(
    ...
    """
    v7: 双空间 in-batch triplet loss，使用 binary_label（0/1）构造三元组。

    相比 v6b 的改动：
      - 原版用 hazard_type 做 triplet 标签，hazard=-1 的 benign 全部被 valid>=0 过滤
      - v7 改用 binary_label，benign(0) 和 harmful(1) 均参与三元组构造
      - benign anchor 找 benign pos + harmful neg，提供明确的正负分离信号

    返回: (loss, sp_emb, sn_emb, sp_use, sn_use)
    """
    # binary_label 无 -1，所有样本均有效
    ze = F.normalize(traj_euc,   p=2, dim=-1)
    zu = F.normalize(traj_usage, p=2, dim=-1)
    y  = binary_labels  # [B]，0 or 1
...
def usage_center_gap(traj_usage: torch.Tensor, labels: torch.Tensor) -> dict:
    """binary (0/1) 类中心 cosine / L2 gap，纯监控用"""
...
def _expand_binary_to_steps(binary: torch.Tensor, step_mask: torch.Tensor) -> torch.Tensor:
    """
    将 trajectory 级 binary [B] 广播到 step 级 [N_valid_steps]。
    用于 loss_recon_weighted 的 flat_binary 参数。
    """
    B, T   = step_mask.shape
    # [B] → [B, T]
...
def evaluate(model, loader, device, config):
    ...
    for batch in tqdm(loader, desc="Eval", leave=False):
        ...
        flat_binary   = _expand_binary_to_steps(binary, enc["step_mask"])  # v7
        ...
        all_langs.extend(batch["langs"])   # v7 新增
        n += 1
...
def train_stage1(config, device):
    ...
    # 冻结除 atom_bank / query_proj 以外的所有参数
